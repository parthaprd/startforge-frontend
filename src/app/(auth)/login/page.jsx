...
export default function LoginPage() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      router.push(redirect || getDashboardRoute(user.role));
    } catch (err) {
  ...
  return (
    <div className="mt-10">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-5">
        ...
        <Input label="Password" type="password" placeholder="••••••••" leftIcon={Lock} error={errors.password?.message} {...register('password')} />
        <div className="flex items-center justify-between">
          <Checkbox label="Remember me" {...register('remember')} />
          <Link href="#" className="text-sm text-primary hover:underline">Forgot password?</Link>
        </div>
        <Button type="submit" fullWidth size="lg" loading={submitting} leftIcon={LogIn} className="rounded-full">Sign In</Button>
      </form>
      ...
      <button type="button" className="btn-base mt-4 w-full border border-hairline bg-canvas px-4 py-2.5 text-sm font-bold text-ink hover:bg-surface-card rounded-full">
